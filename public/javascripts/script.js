...
let landingDetails = document.querySelector('.landingDetails');
let landingStinger= document.querySelector('.landingStinger');

let fyiCase = document.querySelector('.fyiCase');
let fyiReferenceCase = document.querySelector('.fyiReferenceCase');

let fs= document.querySelectorAll('.fs');
let frs= document.querySelectorAll('.frs');

for(let i=0;i<solutions.length;i++){
    solutions[i].onclick=function(){
        fyiReferenceCase.innerHTML=frs[i].innerHTML;
        fyiCase.innerHTML=fs[i].innerHTML;
        details[i].classList.replace('hide','.show');
        landingDetails.classList.add('hide');
        landingStinger.classList.add('hide');
        let otherDetails=Object.values(details).filter(value => (value !== details[i]));
